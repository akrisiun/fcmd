    ...
    public class Stylist
    {
        ...
        public Stylist(string CSS_File = null)
        {
            ...
            if (DefaultStyle == null)
                ...
                "File Commander has been crashed: the default theme's stylesheet is unable to load. Possibly there is a failure of the pluginner.dll body or RAM banks. Try to reinstall FC.", new InvalidProgramException("Default style isn't loading"));
        ...
        public void Stylize(Widget Widget, string Selector = "Widget")
        {
            if (!semaphore)
            {
                ...
                {
                    Console.WriteLine("NOTICE: No style is set for widgets of type "
                + Widget.GetType().ToString().Substring(Widget.GetType().ToString().IndexOf('.') + 1));
                }
                semaphore = false;
            }
        ...
        /// <param name="Selector">The selector pattern</param>
        public void Stylize(IListView2 Widget, string Selector = "FileList")
        {
