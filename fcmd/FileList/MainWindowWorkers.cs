...
namespace fcmd
{
    public static class MainWindowWorkers
    {
            ...
            ref ReplaceQuestionDialog.ClickedButton Feedback, AsyncCopy AC)
        {
            if (SourceURL == DestinationURL)
            {
                ...
                string toshow = string.Format(Localizator.GetString("CantCopy"),
                    SourceFS.GetMetadata(SourceURL).Name, itself);
            ...
            if (DestinationFS.FileExists(DestinationURL))
            {
                ...
                Xwt.Application.Invoke(
                    delegate
                    {
                        rpd = new ReplaceQuestionDialog(
                            DestinationFS.GetMetadata(DestinationURL).Name);
                        ready = true;
                    }
                );
                do { } while (!ready);
        ...
        /// </summary>
        public static void DoCpDir(this MainWindow @this, string source,
            string destination, pluginner.IFSPlugin fsa, pluginner.IFSPlugin fsb)
        {
            ...
            foreach (DirItem di in fsa.DirectoryContent)
            {
                ...
                {
                    //it is subdirectory
                    @this.DoCpDir(di.URL, destination + fsb.DirSeparator + di.TextToShow, fsa, fsb);
                }
            }
        }
        ...
        public static void DoRmFile(this MainWindow @this, string url, pluginner.IFSPlugin fs)
        {
            try
            {
                fs.DeleteFile(url);
        ...
        /// <param name="url">url of the file</param>
        /// <param name="fs">filesystem of the file</param>
        public static void DoRmDir(this MainWindow @this, string url, pluginner.IFSPlugin fs)
        {
            try
            {
